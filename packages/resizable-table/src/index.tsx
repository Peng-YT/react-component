...
const ResizableHeaderCell: React.FC<ResizableHeaderCellPropsType> = ({
    onWidthChange,
    title,
    dataKey,
    dataWidth,
    ...
    const onResize = (e) => {
        ...
        if (lineEl.current) lineEl.current.style.transform = `translateX(${prevMoveX.current}px)`;
    };
    const onResizeStop = () => {
...
const BodyCell: React.FC<BodyCellPropsType> = ({
    dataKey,
    dataWidth,
    xScroll,
    children,
...
function ResizableTable<RecordType extends object = any>({
    ...
    const handleColumns = (columns: ColumnsType<RecordType>) => {
